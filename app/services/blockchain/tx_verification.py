...
class EvmTxVerifier:
    def __init__(
        ...
        *,
        ...
        recipient_wallets: dict[str, str],
        ...
        self._recipient_wallets = {k.lower(): _normalize_address(v) for k, v in recipient_wallets.items()}
    ...
    def verify_transfer(
        ...
        *,
        ...
        recipient = self._recipient_wallets.get(network)
        if not recipient:
            return BlockchainVerificationResult(
                ok=False,
                reason=BlockchainVerificationReason.NETWORK_MISMATCH,
                note=f"Missing expected recipient wallet for network {network}",
            )
        ...
        transfer = _extract_transfer(tx=tx, receipt=receipt, expected_token=expected_token)
        if transfer is None:
            return BlockchainVerificationResult(
                ok=False,
        ...
        normalized_token = transfer.get("token")
        if expected_token and normalized_token and normalized_token.lower() != expected_token.lower():
            return BlockchainVerificationResult(
                ok=False,
                reason=BlockchainVerificationReason.TOKEN_MISMATCH,
                note=f"Unexpected token {normalized_token}, expected {expected_token}",
            )
...
def _extract_transfer(*, tx: dict[str, Any], receipt: dict[str, Any], expected_token: str | None) -> dict[str, Any] | None:
    expected_token_normalized = _normalize_address(expected_token) if expected_token else None

    if expected_token_normalized:
        logs = receipt.get("logs") or []
        for log in logs:
            if _normalize_address(log.get("address")) != expected_token_normalized:
                continue
            ...
            amount = _hex_to_decimal(log.get("data"), decimals=18)
            return {"recipient": recipient, "amount": amount, "token": expected_token_normalized}
        return None
